...
buf.Append($"// on {DateTime.UtcNow.ToString("o")}, from {ucd.Description}, {totalSize} bytes\n");
buf.Append("// clang-format off\n");

foreach (var (s, i) in trie.Stages.Select((v, i) => (v, i)))
{
    var fmt = $" 0x{{0:x{s.Bits / 4}}},";
    var width = 16;
    if (i != 0)
    {
        width = s.Mask + 1;
    }
    buf.Append($"static constexpr uint{s.Bits}_t s_stage{i + 1}[] = {{");
    foreach (var (value, j) in s.Values.Select((v, j) => (v, j)))
    {
        if (j % width == 0)
        {
            buf.Append("\n   ");
...
foreach (var (s, i) in trie.Stages.Select((v, i) => (v, i)))
{
    buf.Append($"    const auto s{i + 1} = s_stage{i + 1}[");
    if (i == 0)
    {
        buf.Append($"cp >> {s.Shift}");
    }
    else
    {
        buf.Append($"s{i} + ((cp >> {s.Shift}) & {s.Mask})");
    }
    buf.Append("];\n");
}
buf.Append($"    return s{trie.Stages.Count};\n");
buf.Append("}\n");
...
static Trie BuildTrie(List<TrieType> uncompressed, Span<int> shifts)
{
    ...
    foreach (var shift in shifts)
    {
        ...
        for (var i = 0; i < uncompressed.Count; i += chunkSize)
        {
            var key = new ReadOnlyTrieTypeSpan(uncompressed, i, Math.Min(chunkSize, uncompressed.Count - i));
...
public class ReadOnlyTrieTypeSpan
{
    private readonly List<TrieType> _list;
    private readonly int _start;
    private readonly int _length;
    private readonly TrieType[] _data;

    public ReadOnlyTrieTypeSpan(List<TrieType> list, int start, int length)
    {
        _list = list;
        _start = start;
        _length = length;
        _data = AsSpan().ToArray();
    }

    public ReadOnlySpan<TrieType> AsSpan() => CollectionsMarshal.AsSpan(_list).Slice(_start, _length);
