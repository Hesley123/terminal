...
// on 2024-05-27T17:21:43Z, from Unicode 15.1.0, 8491 bytes
// clang-format off
...
constexpr uint8_t ucdLookup(const char32_t cp) noexcept
{
    ...
    const auto s1 = s_stage1[s1 + ((cp >> 6) & 31)];
    const auto s2 = s_stage2[s2 + ((cp >> 3) & 7)];
    const auto s3 = s_stage3[s3 + ((cp >> 0) & 7)];
    return s3;
}
